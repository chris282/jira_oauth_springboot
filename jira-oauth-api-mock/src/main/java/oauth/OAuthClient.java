...
import static oauth.PropertiesClient.ACCESS_TOKEN;
import static oauth.PropertiesClient.CONSUMER_KEY;
import static oauth.PropertiesClient.PRIVATE_KEY;
import static oauth.PropertiesClient.REQUEST_TOKEN;
import static oauth.PropertiesClient.SECRET;
...
public class OAuthClient {
	...
	private Optional<Exception> handleGetRequestTokenAction(List<String> arguments) {
		System.out.println("[jira-oauth-mock] handleGetRequestTokenAction()");
		Map<String, String> properties = propertiesClient.getPropertiesOrDefaults();
		printPropertiesMap(properties);
		try {
			...
			printPropertiesMap(propertiesClient.getPropertiesOrDefaults());
			return Optional.empty();
		} catch (Exception e) {
	...
	private Optional<Exception> handleGetAccessToken(List<String> arguments) {
		System.out.println("[jira-oauth-mock] handleGetAccessToken()");
		Map<String, String> properties = propertiesClient.getPropertiesOrDefaults();
	...
	private Optional<Exception> handleGetRequest(List<String> arguments) {
		System.out.println("[jira-oauth-mock] handleGetRequest()");
		Map<String, String> properties = propertiesClient.getPropertiesOrDefaults();
		...
		try {
			...
			String signatureMethod= parameters.signatureMethod;
			System.out.println("[jira-oauth-mock] handleGetRequest() signatureMethod="+signatureMethod);
			//  parameters.setOAuthType(OAuthParameters.OAuthType.TWO_LEGGED_OAUTH);
	...
	private void parseResponse(HttpResponse response) throws IOException {
		System.out.println("[jira-oauth-mock] parseResponse()");
	...
	private static HttpResponse getResponseFromUrl(OAuthParameters parameters, GenericUrl jiraUrl) throws IOException {
		System.out.println("[jira-oauth-mock] getResponseFromUrl() ");
		HttpRequestFactory requestFactory = new NetHttpTransport().createRequestFactory(parameters);
		HttpRequest request = requestFactory.buildGetRequest(jiraUrl);
		return request.execute();
	}

	private void printPropertiesMap(Map<String,String> properties) {
		for (Map.Entry<String,String> entry : properties.entrySet()){
			System.out.println("map entry : key="+entry.getKey()+" , "+entry.getValue());
		}
	}
}
